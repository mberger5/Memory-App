...
def reset_quiz_state() -> None:
    for key in ["question", "answered", "last_choice", "mode_for_question"]:
...
def main() -> None:
    ...
    accuracy = (st.session_state.score / st.session_state.attempts * 100) if st.session_state.attempts else 0
    ...
    for i, option in enumerate(question.options):
        if st.button(option, key=f"option_{i}_{option}", disabled=disabled, use_container_width=True):
    ...
    with st.expander("Saved quiz history"):
        hist = history_df()
        if hist.empty:
            st.write("No saved history yet. On hosted Streamlit, saved history may reset when the app restarts.")
        else:
